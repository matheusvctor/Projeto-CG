...
class Viewport:
    def __init__(self, largura=900, altura=900, escala=1):  # zoom padrão = 1
        self.largura = int(largura)
        self.altura  = int(altura)
        self.escala  = max(1, int(escala))  # px por unidade
...
class QuadroDesenho:
    def __init__(self, canvas: tk.Canvas, vp: Viewport, usar_grade=True):
        ...
        self.cxw = 0.0  # centro do mundo visível
        self.cyw = 0.0
...
def circunferencia_equacao(xc, yc, r, cor="#000"):
    ...
    for x in range(0, r + 1):
        ...
        # Espelhando apenas para os 4 quadrantes (X e Y independentes)
...
def circunferencia_trigonometrica(xc, yc, r, cor="#000"):
    # BURACOS: Varremos de 0 a 360 graus (2 PI) diretamente.
    if r <= 0: return
    passo = 1.0 / r
    t = 0.0
    fim = 2 * math.pi
...
def _sim4(xc, yc, x, y, cor="#000"):
    # Simetria de 4 pontos para a Elipse
    set_pixel(xc + x, yc + y, cor)
    set_pixel(xc - x, yc + y, cor)
    set_pixel(xc + x, yc - y, cor)
    set_pixel(xc - x, yc - y, cor)
...
def elipse_ponto_medio(xc, yc, rx, ry, cor="#000"):
    ...
    # Região 1 (Onde o eixo X cresce mais rápido que o Y)
    ...
    # Região 2 (Onde o eixo Y passa a decrescer mais rápido)
...
def curva_bezier_cubica(p0, p1, p2, p3, passos=100, cor="#000"):
    pts = []
    # Calcula os pontos baseados na equação paramétrica de Bézier [0 <= t <= 1]
    ...
    # Conecta as amostras usando o nosso motor DDA para garantir linha contínua
...
def secao_conica_varredura(a, b, c, d, e, f, cor="#000000", margem=2, max_gap=5):
    xmin, xmax, ymin, ymax = _limites_inteiros_visiveis(margem)
...
def T(tx,ty): return [[1,0,tx],[0,1,ty],[0,0,1]]
...
def seg_origem(L):        return [(0,0),(L,0)]
def quadrado_origem(Sz):  return [(0,0),(Sz,0),(Sz,Sz),(0,Sz)]
def triangulo_origem(Sz): return [(0,0),(Sz,0),(0,Sz)]
