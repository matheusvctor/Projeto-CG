...
from core.cg_utils import *  # Viewport, QuadroDesenho, raster, etc.
...
class AppFormas:
    def __init__(self, root, on_back=None): 
        ...
        self.ca=tk.DoubleVar(value=1.0); self.cb=tk.DoubleVar(value=1.0)
        self.cc=tk.DoubleVar(value=0.0); self.cd=tk.DoubleVar(value=0.0)
        self.ce=tk.DoubleVar(value=0.0); self.cf=tk.DoubleVar(value=-1600.0)
        ...
        # ---- Painel direito com Notebook: Resumo + Como calculamos
        ...
        # ---- Esquerda: lista de pontos (amostras)
    ...
    def _rebuild_inputs(self):
        ...
        elif alg == "Seção Cônica (Geral)":
            self._row("A, B:", self.ca, self.cb)
            self._row("C, D:", self.cc, self.cd)
            self._row("E, F:", self.ce, self.cf)
            ttk.Label(
                self.param_frame,
                text="Ax² + By² + Cxy + Dx + Ey + F = 0",
    ...
    def _help_text(self, alg):
        ...
        if alg == "Seção Cônica (Geral)":
            return ("Seção cônica geral\n"
                    "• equação: Ax² + By² + Cxy + Dx + Ey + F = 0\n"
                    "• discriminante: Δ = C² − 4AB\n"
                    "• Δ < 0: elipse/circunferência | Δ = 0: parábola | Δ > 0: hipérbole\n"
                    "• varredura no retângulo visível, resolvendo por x e por y")
    ...
    def _registrar(self, msg): self.log.insert(tk.END, msg + "\n"); self.log.see(tk.END)
    ...
    def desenhar(self):
        ...
        try:
            if   alg == "DDA (Reta)":
                ...
                a, b, c, d, e, f = (
                    self.ca.get(), self.cb.get(), self.cc.get(),
                    self.cd.get(), self.ce.get(), self.cf.get(),
                )
                tipo, discriminante = classificar_secao_conica(a, b, c, d, e, f)
                self._registrar(
                    f"equação: {a:g}x² + {b:g}y² + {c:g}xy + {d:g}x + {e:g}y + {f:g} = 0"
                )
                self._registrar(f"Δ = C² - 4AB = {discriminante:.3f} -> {tipo}")
                self._registrar("varredura: resolvendo a curva por x e por y na área visível")
                secao_conica_varredura(a, b, c, d, e, f)
                self._last=("conica", (a, b, c, d, e, f))
            elif alg == "Curva de Bézier":
...
def main():
    ...
    app = AppFormas(root)
    # menu decide o tamanho; aqui sem fullscreen por padrão
    root.mainloop()
