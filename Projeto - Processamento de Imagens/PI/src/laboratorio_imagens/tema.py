...
def encontrar_arquivo_real(alvo: str | Path) -> Path | None:
    ...
    if "__file__" in globals():
        ...
        locais_iniciais.insert(0, Path(__file__).resolve().parent.parent.parent)
    ...
    for base in locais_iniciais:
        if (base / "Projeto - Processamento de Imagens").exists() or (base / "Projeto 1 - Computação Gráfica").exists() or "Computacao-grafica-main" in str(base):
...
def abrir_na_ide(caminho_relativo: str | Path, linha: int = 1) -> None:
    ...
    # 1. Executa CLI do VS Code / Antigravity / Cursor com goto na linha (-g)
    try:
    ...
    # 3. Fallback do Windows para abrir o arquivo existente
    try:
